    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.courses.show', $course) }}" class="text-blue-400 hover:text-blue-300 transition">
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800/60 rounded-xl border border-gray-700/50 p-6">
                <div class="flex items-center gap-4 mb-6 pb-4 border-b border-gray-700/50">
                    <div class="w-12 h-12 rounded-lg bg-green-500/15 flex items-center justify-center text-green-400">
                        <i class="fas fa-graduation-cap text-xl"></i>
                    </div>
                    <div>
                        <h3 class="text-lg font-semibold text-white">{{ $course->title }}</h3>
                        <p class="text-sm text-gray-400">Course #{{ $course->id }} • {{ $course->instructor->name ?? 'No Instructor' }}</p>
                    </div>
                </div>
                ...
                <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" class="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition" value="{{ old('title', $course->title) }}" required>
                            @error('title')
                        ...
                        <div>
                            ...
                            <input type="text" name="slug" class="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition" value="{{ old('slug', $course->slug) }}" placeholder="auto-generated-from-title">
                            @error('slug')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" class="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition resize-none">{{ old('description', $course->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="price" class="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition" value="{{ old('price', $course->price) }}" step="0.01">
                                @error('price')
                                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="duration_days" class="w-full bg-gray-700/50 border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition" value="{{ old('duration_days', $course->duration_days) }}">
                                @error('duration_days')
                                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex items-center gap-3 pt-2">
                            <input type="checkbox" name="is_published" id="is_published" class="w-4 h-4 accent-blue-600 rounded" {{ old('is_published', $course->is_published) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            @if($course->image)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $course->image) }}" alt="{{ $course->title }}" class="h-20 rounded-lg object-cover">
                                </div>
                            @endif
